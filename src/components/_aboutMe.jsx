...
function AboutMe({ language }) {
  ...
  const timelineEvents = [
    {
      year: 2023,
      month: `${text.aboutMe.projectArticle.date}`,
      event: `${text.aboutMe.projectArticle.description}`,
      link: article,
      isAcademic: true,
    },
    {
    ...
    <div className="relative bg-[#111316] py-8 px-6">
      <div className="mb-8">
        <span className="flex gap-4">
          ...
          <h4 className="font-sans text-white font-bold text-2xl">
            {text.aboutMe.aboutMeText}
          </h4>
        </span>

        <p className="text-gray-400 mt-1 text-sm">{text.aboutMe.description}</p>
      </div>
      ...
      <div className="border-l-2 border-gray-300 pl-6">
        {timelineEvents.map((item, index) => (
          <div className="flex items-start mb-6 relative" key={index}>
            <div
              className={`absolute left-[-8px] w-4 h-4 ${
                item.isAcademic
            ...
            <div className="ml-8">
              <strong className="block text-lg font-semibold text-gray-200">
